...
const COUNT_METRICS = [
  ...
  // A degraded OpenSky sweep is missing some hotspots, so compare only the
  // hotspots that reported aircraft in both sweeps (an empty hotspot means
  // "no observations", not an empty sky).
  { key: 'air_total', extract: d => d.air?.reduce((s, a) => s + a.total, 0) || 0, label: 'Air Activity',
    allowDegraded: true, pair: (curr, prev) => pairedAirTotals(curr, prev) },
...
function metricAvailable(data, key, allowDegraded = false) {
  const name = COUNT_SOURCES[key];
  const health = data.health?.find(h => (h.n || h.name) === name);
  if (!health) return true;
  if (allowDegraded && health.status === 'degraded') return true;
  return !health.err && !health.stale;
}

function pairedAirTotals(current, previous) {
  const observed = d => new Map((d.air || []).filter(a => a.total > 0).map(a => [a.region, a.total]));
  const curr = observed(current);
  const prev = observed(previous);
  let currTotal = 0;
  let prevTotal = 0;
  for (const [region, total] of curr) {
    if (!prev.has(region)) continue;
    currTotal += total;
    prevTotal += prev.get(region);
  }
...
export function computeDelta(current, previous, thresholdOverrides = {}) {
  ...
  for (const m of COUNT_METRICS) {
    if (!metricAvailable(current, m.key, m.allowDegraded) || !metricAvailable(previous, m.key, m.allowDegraded)) continue;
    const [curr, prev] = m.pair ? m.pair(current, previous) : [m.extract(current), m.extract(previous)];
    if (curr == null || prev == null) continue;
  ...
  // European background (BfS + STUK) network-wide rise. `null` means the
  // networks were unavailable, which neither raises nor resolves the alert.
  const currEu = current.radBackground?.anomaly;
  const prevEu = previous.radBackground?.anomaly;
  if (currEu === true && prevEu !== true) {
    const detail = (current.radBackground.networks || [])
      .filter(n => Number.isFinite(n.medianUSvH)).map(n => `${n.network} ${n.medianUSvH.toFixed(2)} µSv/h`).join(', ');
    signals.new.push({ key: 'radiation_eu_anomaly', reason: `European background radiation elevated${detail ? ` (${detail})` : ''}`, severity: 'critical' });
    criticalChanges += 5;
  } else if (currEu === false && prevEu === true) {
    signals.deescalated.push({ key: 'radiation_eu_anomaly', label: 'EU Radiation Background', direction: 'resolved', severity: 'high' });
  }
