 ...
 */
function loadLocale(lang, allowFallback = true) {
  if (localeCache.has(lang)) {
    return localeCache.get(lang);
  }

  // The fallback is only ever taken one level deep, and never for the default
  // locale itself — a missing or corrupt locales/en.json used to recurse into
  // loadLocale('en') forever and blow the stack.
  const fallback = () => {
    if (!allowFallback || lang === DEFAULT_LOCALE) {
      localeCache.set(lang, {});
      return {};
    }
    return loadLocale(DEFAULT_LOCALE, false);
  };
