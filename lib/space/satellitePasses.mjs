...
export async function getSatellitePassContext(question = '', opts = {}) {
  ...
  const withObserver = value => ({ ...value, observer: { ...observer, matched: resolved.matched, requestedZip: resolved.requestedZip } });
  ...
  if (cacheable) {
    ...
    if (hit && hit.expiresAt > Date.now()) return withObserver(hit.value);
  }
