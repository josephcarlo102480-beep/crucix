...
export async function answerDashboardQuestion(provider, dashboardData, question, rawData = null) {
  ...
  const { dashboardJson, rawJson, condensed } = buildAskContext(dashboardData, rawData);
...
RAW_SOURCE_DATA:${rawJson ? ' (full unsynthesized output from every source in the latest sweep — most granular layer)' : ''}
${rawJson || 'Not available for this request.'}
...
//
// Returns the layers already serialized: a full sweep is hundreds of KB, and
// stringifying it once for the size check and again for the prompt doubled both
// the CPU and the peak memory of every Ask AI request.
function buildAskContext(dashboardData, rawData) {
  const serialize = (value) => (value == null ? null : JSON.stringify(value, null, 2));
  const fits = (dash, raw) => (dash?.length || 0) + (raw?.length || 0) <= MAX_CONTEXT_CHARS;

  // 1. Try every layer at full fidelity.
  const dashFullJson = serialize(compactDashboardForAsk(dashboardData, FULL_LIMITS));
  const rawFullJson = rawData != null ? serialize(sanitizeForLLM(rawData, FULL_LIMITS)) : null;
  if (fits(dashFullJson, rawFullJson)) {
    return { dashboardJson: dashFullJson, rawJson: rawFullJson, condensed: false };
  }

  // 2. Condense both layers.
  const dashSmallJson = serialize(compactDashboardForAsk(dashboardData, CONDENSED_LIMITS));
  const rawSmallJson = rawData != null ? serialize(sanitizeForLLM(rawData, CONDENSED_LIMITS)) : null;
  if (fits(dashSmallJson, rawSmallJson)) {
    return { dashboardJson: dashSmallJson, rawJson: rawSmallJson, condensed: true };
  }

  // 3. Last resort: keep the condensed snapshot, drop the raw layer.
  return { dashboardJson: dashSmallJson, rawJson: null, condensed: true };
}
...
export function compactDashboardForAsk(data = {}, limits = FULL_LIMITS) {
  const grouped = {
    ...
    mapLayers: {
      ...
      // Summary only: the ~600 thinned map points add nothing a question needs.
      radiationBackground: data.radBackground && (({ stations, ...summary }) => summary)(data.radBackground),
      epa: data.epa,
      noaa: data.noaa,
      sdr: data.sdr,
      space: data.space,
  ...
  const groupedKeys = new Set([
    ...
    'newsFeed', 'news', 'who', 'gdelt',
    'air', 'airMeta', 'thermal', 'chokepoints', 'nuke', 'nukeSignals', 'radBackground',
