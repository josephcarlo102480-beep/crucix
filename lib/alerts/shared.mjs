...
export const ALERT_TIERS = ['FLASH', 'PRIORITY', 'ROUTINE'];

/**
 * An LLM answer is only usable if it decides, and — when it decides to alert —
 * carries the fields the embed is built from. A half-filled alert would post an
 * embed reading "undefined", so anything short of complete falls back to rules.
 * A well-formed "do not alert" is a real decision and is kept as-is.
 */
export function isValidAlertEvaluation(evaluation) {
  if (!evaluation || typeof evaluation !== 'object') return false;
  if (typeof evaluation.shouldAlert !== 'boolean') return false;
  if (!evaluation.shouldAlert) return true;

  const nonEmpty = (v) => typeof v === 'string' && v.trim().length > 0;
  if (!nonEmpty(evaluation.headline) || !nonEmpty(evaluation.reason)) return false;
  return ALERT_TIERS.includes(String(evaluation.tier || '').trim().toUpperCase());
}
...
export async function evaluateAlertDecision({ llmProvider, signals, delta, logLabel }) {
  ...
  if (llmProvider?.isConfigured) {
    try {
      ...
      if (evaluation && !isValidAlertEvaluation(evaluation)) {
        console.warn(`[${logLabel}] LLM alert JSON incomplete (headline/reason/tier), falling back to rules`);
        evaluation = null;
      } else if (evaluation?.shouldAlert) {
        evaluation.tier = String(evaluation.tier).trim().toUpperCase();
      }
    } catch (err) {
  ...
  if (!isValidAlertEvaluation(evaluation)) {
...
export function ruleBasedEvaluation(signals, delta) {
  ...
  const euRadSignal = signals.find((s) => s.key === 'radiation_eu_anomaly');
  ...
  if (euRadSignal) {
    return {
      ...
      tier: 'FLASH',
      confidence: 'HIGH',
      headline: 'European Background Radiation Elevated',
      reason: `${euRadSignal.reason}. A network-wide rise across state dose-rate monitors, not a single probe.`,
      actionable: 'Check the EU Radiation Background layer for the affected region. Look for confirmation from national agencies.',
      signals: ['radiation_eu_anomaly'],
      crossCorrelation: 'radiation monitors',
    };
  }

  const hasCriticalMarket = criticals.some((s) => marketSignals.includes(s));
  const hasCriticalConflict = criticals.some((s) => conflictSignals.includes(s));
...
export function buildEvaluationPrompt() {
  return `You are Crucix, an elite intelligence alert evaluator for a personal OSINT monitoring system. You analyze signal deltas from a 26-source intelligence sweep and decide if the user needs to be alerted.
...
- Social media noise without hard-data confirmation (social chatter alone is NOT enough)
...
- New conflict front or ceasefire collapse confirmed by ACLED + corroborating reporting
...
export function buildSignalContext(signals, delta) {
  ...
  const conflictSignals = signals.filter((s) => ['conflict_events', 'conflict_fatalities', 'thermal_total'].includes(s.key));
  const otherSignals = signals.filter((s) => !marketSignals.includes(s) && !conflictSignals.includes(s));
